  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-40 border-b bg-primary">
        <div className="max-w-[1440px] mx-auto px-6 min-h-14 flex items-center gap-4 overflow-hidden">
          ...
          {/* Tab navigation */}
          <div className="flex flex-1 min-w-0 items-center gap-1 bg-primary-foreground/10 rounded-lg p-0.5 overflow-x-auto">
            {tabs.map(t => (
              <button
                key={t.key}
                onClick={() => setActiveTab(t.key)}
                className={`shrink-0 px-4 py-1.5 rounded-md text-xs font-medium transition-all ${
                  activeTab === t.key
          ...
          <span className="hidden 2xl:block shrink-0 text-xs text-primary-foreground/50">338 Units · 9 Buildings · 3 Concepts</span>
        </div>
      </header>
